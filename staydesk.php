...
/**
 * Remove page titles from browser tabs for StayDesk pages.
 * Sets a simple, clean title without the "- StayDesk" suffix.
 */
function staydesk_remove_page_titles($title_parts) {
    if (is_page()) {
        $page_slug = get_post_field('post_name', get_post());
        $staydesk_pages = array(
            'staydesk-home', 'staydesk-login', 'staydesk-signup', 'staydesk-verify-email',
            'staydesk-dashboard', 'staydesk-rooms', 'staydesk-bookings', 'staydesk-profile',
            'staydesk-pricing', 'staydesk-forgot-password', 'staydesk-admin', 'staydesk-settings'
        );
        if (in_array($page_slug, $staydesk_pages)) {
            // Set a minimal title - just the site name
            $title_parts['title'] = 'StayDesk';
            unset($title_parts['tagline']);
        }
    }
    return $title_parts;
}
add_filter('document_title_parts', 'staydesk_remove_page_titles', 99);
