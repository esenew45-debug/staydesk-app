...
class Staydesk_Admin {
    ...
    public function render_admin_dashboard() {
        // Check if user is logged in with staydesk_hotel role
        if (!is_user_logged_in() || !current_user_can('staydesk_hotel')) {
            return '<p>Please <a href="' . home_url('/staydesk-login') . '">login</a> to access the dashboard.</p>';
        }
        
        // Check if user has admin/manager role in StayDesk system
        if (!session_id()) {
            session_start();
        }
        
        $user_role = isset($_SESSION['staydesk_user_role']) ? $_SESSION['staydesk_user_role'] : '';
        
        // Allow access for hotel_manager (admin) role
        // Staff can also view dashboard but with limited functionality
        if (empty($user_role) || !in_array($user_role, array('hotel_manager', 'hotel_staff'))) {
            // Fallback: check if user has the role in database
            global $wpdb;
            $table_roles = $wpdb->prefix . 'staydesk_roles';
            $user_id = get_current_user_id();
            
            $role_record = $wpdb->get_row($wpdb->prepare(
                "SELECT role_name FROM $table_roles WHERE user_id = %d ORDER BY id DESC LIMIT 1",
                $user_id
            ));
            
            if ($role_record && in_array($role_record->role_name, array('hotel_manager', 'hotel_staff'))) {
                $user_role = $role_record->role_name;
                $_SESSION['staydesk_user_role'] = $user_role;
            } else {
                return '<p>Access denied. You need admin or staff privileges to access this page.</p>';
            }
        }
