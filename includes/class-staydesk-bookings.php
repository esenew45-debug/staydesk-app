...
class Staydesk_Bookings {
    ...
    public function update_booking_status() {
        ...
        if (!$existing_booking) {
            error_log('Booking not found. Booking ID: ' . $booking_id);
            wp_send_json_error(array('message' => 'Booking not found.'));
        }
        
        error_log('Current booking status: ' . $existing_booking->booking_status . ', Requested status: ' . $status);
        ...
        // Note: $wpdb->update() returns false on error, 0 if no rows affected, or number of rows affected
        if ($result === false) {
            error_log('Database error updating booking status. Booking ID: ' . $booking_id . ', Status: ' . $status . ', Error: ' . $wpdb->last_error);
            wp_send_json_error(array('message' => 'Database error: ' . $wpdb->last_error));
        }

        error_log('Booking status updated successfully. Rows affected: ' . $result . ', Booking ID: ' . $booking_id . ', New Status: ' . $status);
        wp_send_json_success(array(
