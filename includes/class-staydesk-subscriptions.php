...
class Staydesk_Subscriptions {
    ...
    public function upgrade_subscription() {
        ...
        // Get current subscription plan (from hotel record or subscription table)
        $current_plan = $hotel->subscription_plan ?: 'monthly';
        
        if ($current_plan === 'yearly') {
            wp_send_json_error(array('message' => 'You are already on a yearly plan.'));
        }

        // Check if hotel has an active subscription
        if ($hotel->subscription_status !== 'active' || empty($hotel->subscription_expiry)) {
            wp_send_json_error(array('message' => 'No active subscription found. Please subscribe first.'));
        }
